...
public class ChassisSubsystem extends SubsystemBase {
  ...
  public void drive(RobotContainer robotContainer, DriverOI driverOI, int scale) {
    ...
    dir =
        driverOI.getJoystick().getRawButton(Constants.LEFT_BUMPER)
            ? Constants.FORWARD
            : Constants.REVERSE;
    ...
    if (driveTrainType == RobotContainer.DriveTrainType.TANK) {
      ...
      if (dir == Constants.REVERSE) {
        rightVal =
            getScaledValue(
                driverOI.getJoystick().getRawAxis(Constants.RIGHT_JOYSTICK_Y),
                scale,
                RobotSide.RIGHT);
        leftVal =
            getScaledValue(
                driverOI.getJoystick().getRawAxis(Constants.LEFT_JOYSTICK_Y),
                scale,
                RobotSide.LEFT);
      } else {
        rightVal =
      ...
      double rotation =
          getScaledValue(
              driverOI.getJoystick().getRawAxis(Constants.LEFT_JOYSTICK_X), scale, RobotSide.RIGHT);
